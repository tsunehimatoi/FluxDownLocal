...
版本命名规则：采用 `v<UpstreamVersion>-local.<N>`（如 `0.4.7-local.1`），`<UpstreamVersion>` 取自上游基准版本，`-local.<N>` 为本地精简版构建/迭代序号。详见 [`.omp/knowledge/upstream-sync.md` §3.5](upstream-sync.md)。
...
```bash
# 1. 依据 upstream-sync.md §3.5 确定上游基准版本（例如 0.4.7）
# 2. 确保 pubspec.yaml、wxt.config.ts 等已对齐版本号（例如 version: 0.4.7-local.1+1）
rinf gen
...
# 2. 压缩 Chrome 与 Firefox 包（统一遵循 0.4.7-local.1 命名）
pwsh -Command @"
...
pwsh -File scripts\build_custom_windows.ps1 -Version 0.4.7-local.1 -OutputDirectory dist

# 制作绿色免安装便携版 zip
pwsh -Command "Compress-Archive -Path 'build\windows\x64\runner\Release\*' -DestinationPath 'dist\FluxDown-0.4.7-local.1-windows-x64-portable.zip' -Force"
```
